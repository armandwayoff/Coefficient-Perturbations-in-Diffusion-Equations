...
%% Code
function [hh,varargout] = ffpdeplotbis(points,boundary,triangles,varargin)
    ...
    optsnames = {'VhSeq', 'XYData', 'XYStyle', 'Mesh', 'MColor', 'Boundary', 'BDLabels', ...
                 ...
                 'ColorMap', 'ColorBar', 'CBPosition', 'ColorRange', ...
    ...
    [vhseq, xyrawdata, xystyle, showmesh, mcolor, showboundary, bdlabels, ...
     ...
     setcolormap, showcolbar, colorbarposition, colorrange, ...
    ...
    if ~isempty(xyrawdata)
            ...
            if (isnumeric(colorrange))
                ...
                if strcmpi(colorrange,'minmax') || strcmpi(colorrange,'centered')
                    ...
                    if strcmpi(colorrange,'minmax')
                        if min(min(cdata)) == max(max(cdata));
                          caxis([min(min(cdata))-0.5 min(min(cdata))+0.5]);
                        else
        ...
        if ~(strcmpi(showcolbar,'off'))
            if strcmpi(showcolbar,'on')
                if strcmpi(colorbarposition,'on')
                  hcb=colorbar(colorbarposition); 
                  set(hcb, 'FontSize',16);       
                  x_min = min(min(cdata));
                  x_max = max(max(cdata));
                  x_mean = (x_max + x_min) / 2; % mean(mean(cdata));
                  x_ticks = [x_min, x_max];
                  % x_labels = {sprintf('Min: %.5f', x_min), sprintf('Max: %.5f', x_max), sprintf('Mean: %.2f', x_mean)};
                  % x_labels = {'a_{min}', 'a_{max}'};
                  % Set the x-ticks and x-tick labels
                  set(hcb, 'XTick', x_ticks);    
                  % set(hcb, 'XTickLabel', x_labels); 
                end               
            else
